...
function AppContent() {
  ...
  const NavItem = ({ icon: Icon, label, screen }: { icon: any, label: string, screen: Screen }) => {
    if (!Icon) return null;
    const isActive = currentScreen === screen || (screen === 'dashboard' && !['directory', 'calendar', 'settings', 'roadmap', 'governance', 'projects', 'membership', 'finance', 'university'].includes(currentScreen));
    return (
      <button
  ...
  return (
    <div className="min-h-screen bg-slate-50 max-w-md mx-auto shadow-2xl relative overflow-hidden">
      ...
      {!isAuthScreen && currentScreen !== 'dashboard' && !['directory', 'calendar', 'settings'].includes(currentScreen) && (
        <button
          onClick={() => setCurrentScreen('settings')} // Should ideally go back to dashboard or previous? Legacy went to settings? Weird. Let's fix to dashboard.
